...
file_name <- "../../../interim_documents/backups_google_docs/Ocean Solutions Tables_2017-05-15d.xlsx"
```
...
dat <- read_xlsx(path = file_name,
  ...
  mutate_each(funs(substr(., 1, 1)), 2:9) %>%
  mutate_each(funs(as.integer), 2:9)
...
colnames(dat) <- c("Criteria", "Renewables", "Vegetation (g)", "Productivity",
                    "Alkalinity (g)", "CO2 storage", "Others", "Cloud", "Albedo")
# long form
dat_long <- gather(data = dat, key = Solution, value = score, 2:9) %>%
...
fig_t1a
ggsave(filename = "../figs/fig_t1a.pdf", plot = fig_t1a, width = 25, height = 25, units = "cm")
```
...
dat$Criteria[dat$Criteria == "Effectiveness to moderate warming"] <-  "Moderate warming"
dat$Criteria[dat$Criteria == "Effectiveness to moderate acidification (upper ocean/global)"] <-  "Moderate acidification"
dat$Criteria[dat$Criteria == "Effectiveness to moderate relative sea level rise"] <-  "Moderate relative\nsea level rise"
dat$Criteria[dat$Criteria == "Time commitment"] <-  "Time\ncommitment"
dat$Criteria[dat$Criteria == "Technological readiness"] <-  "Technological\nreadiness"
dat$Criteria[dat$Criteria == "Lead time until effective"] <-  "Lead\ntime"
...
fig_t1b
ggsave(filename = "../figs/fig_t1b.pdf", plot = fig_t1b, width = 25, height = 25, units = "cm")
```
...
dat_long <- gather(data = dat, key = Driver, value = score, 2:4) %>%
  dplyr::mutate(Solution = factor(Driver, levels = unique(as.character(Driver)))) %>%
...
fig_t3_2 <- ggplot(data = dat_long, 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
fig_t3_2
ggsave(filename = "../figs/fig_t3_2.pdf", plot = fig_t3_2, width = 25, height = 25, units = "cm")
```
